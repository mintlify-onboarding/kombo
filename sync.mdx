...
[manually trigger a sync](/reference/post%5Fforce-sync)). When you call our API,
we will simply read from this database, resulting in endpoints that always
respond reliably and quickly.

Through our syncing approach, we can also provide [webhooks](/docs/webhooks) for
systems that don't provide webhooks themselves.

<Note>
  Note that syncs adhere to [your scope config](/docs/scopes), so if there are
